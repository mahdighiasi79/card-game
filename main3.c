...
void EnQueue(Node element) {
    ...
    for (int i = frontier.length - 1; i > 0; i--) {

        if (frontier.nodes[i].f <= frontier.nodes[i - 1].f)
            break;

        swap = frontier.nodes[i];
        frontier.nodes[i] = frontier.nodes[i - 1];
        frontier.nodes[i - 1] = swap;
    }
}
...
int h(Node node) {
    ...
    for (int i = 0; i < k; i++) {
        ...
        for (int j = 1; j < node.stacks[i].length - 1; j++) {

            if ((node.stacks[i].cards[j].number == (n - j)) &&
                (node.stacks[i].cards[j].color == node.stacks[i].cards[j + 1].color))
                consistentCards++;
            else
                break;
        }
    ...
    int result = (m * n) - consistentCards;
    return result;
}

Node A_Star(Node initial) {
